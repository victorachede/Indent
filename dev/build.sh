...
export APP_NAME="Indent"
export ASSETS_REPOSITORY="Indent/indent"
export BINARY_NAME="codium"
export CI_BUILD="no"
export GH_REPO_PATH="Indent/indent"
export ORG_NAME="Indent"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="Indent/indent-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-indent
    else
      echo "{}" > ~/.gyp/include.gypi.pre-indent
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-indent ~/.gyp/include.gypi
  fi
